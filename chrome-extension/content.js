...
function CN_ConvertTTSElevenLabs() {
	...
	console.log(
		'[ELEVENLABS] Converting following text segment to audio using model ' +
			model +
			' and voice ' +
			voiceId +
			': ' +
	...
	if (
		...
		try {
			voice_settings['stability'] = parseFloat(
				CN_TTS_ELEVENLABS_STABILITY
			);
			voice_settings['similarity_boost'] = parseFloat(
				CN_TTS_ELEVENLABS_SIMILARITY
			);
		} catch (e) {
			voice_settings = {
		...
		// Control values
		if (
			voice_settings['stability'] === null ||
			voice_settings['stability'] < 0 ||
			voice_settings['stability'] > 1 ||
			isNaN(voice_settings['stability'])
		)
			voice_settings['stability'] = 0;
		if (
			voice_settings['similarity_boost'] === null ||
			voice_settings['similarity_boost'] < 0 ||
			voice_settings['similarity_boost'] > 1 ||
			isNaN(voice_settings['similarity_boost'])
		)
			voice_settings['similarity_boost'] = 0;
	...
	xhr.onreadystatechange = function () {
		if (xhr.readyState === XMLHttpRequest.DONE) {
			try {
				...
				try {
					if (status !== 200) {
						...
						if (
							...
							typeof result.detail.status != 'undefined'
						) {
							...
							if (
								...
								setTimeout(function () {
									console.log(
										'[ELEVENLABS] Too many concurrent requests, waiting a bit'
									);
				...
				alert(
					'Error with ElevenLabs API text-to-speech conversion: ' +
						e.toString()
				);
			}
		}
...
function CN_ContinueElevenLabsPlaybackQueue(situation) {
	...
	for (var i in CN_TTS_ELEVENLABS_QUEUE) {
		if (
			CN_TTS_ELEVENLABS_QUEUE[i].converted &&
			!CN_TTS_ELEVENLABS_QUEUE[i].played
		) {
			obj = CN_TTS_ELEVENLABS_QUEUE[i];
			objIndex = i;
			break;
...
function CN_CheckNewMessages() {
	...
	if (currentMessageCount > CN_MESSAGE_COUNT) {
		// New message!
		console.log(
			'New message detected! current message count: ' +
				currentMessageCount
		);
	...
	if (CN_CURRENT_MESSAGE && CN_CURRENT_MESSAGE.length) {
		var currentText =
			jQuery('.text-base:last').find('.items-start').text() + '';
		////console.log("currentText:" + currentText);

		// Remove code blocks?
		if (CN_IGNORE_CODE_BLOCKS) {
			currentText = jQuery('.text-base:last')
				.find('.items-start')
				.find('.markdown')
				.contents()
				.not('pre')
...
function CN_SendMessage(text) {
	...
	var sendButton = textarea
		.closest('div')
		.find("button[data-testid='send-button'], button");
...
function CN_StartSpeechRecognition() {
	...
	CN_SPEECHREC.onresult = (event) => {
		...
		console.log(
			"[SPEECH-REC] Voice recognition transcript: '" +
				final_transcript +
				"'"
		);
		...
		if (
			CN_RemovePunctuation(final_transcript) ==
			CN_SAY_THIS_WORD_TO_STOP.toLowerCase().trim()
		) {
			if (CN_CONVERSATION_SUSPENDED) {
				console.log(
					'[SPEECH-REC] Conversation is currently suspended, voice command ignored. Use the pause word to resume conversation.'
				);
				return;
			}

			console.log(
				"[SPEECH-REC] [STOP-WORD] You said '" +
			...
			CN_RemovePunctuation(final_transcript) ==
				CN_SAY_THIS_WORD_TO_PAUSE.toLowerCase().trim() || // Below: allow to say the pause word twice
			CN_RemovePunctuation(final_transcript) ==
				CN_SAY_THIS_WORD_TO_PAUSE.toLowerCase().trim() +
					' ' +
					CN_SAY_THIS_WORD_TO_PAUSE.toLowerCase().trim()
		) {
			...
			if (CN_CONVERSATION_SUSPENDED) {
				console.log(
					"[SPEECH-REC] [PAUSE-WORD] You said '" +
			...
			if (CN_CONVERSATION_SUSPENDED) {
				console.log(
					'[SEND-WORD] [SUSPENDED] Conversation is currently suspended, voice command ignored. Use the pause word to resume conversation.'
				);
				return;
			}
		...
		if (CN_CONVERSATION_SUSPENDED) {
			console.log(
				'[IGNORED] Conversation is currently suspended, voice command ignored. Use the pause word to resume conversation.'
			);
			return;
		}
...
function CN_ToggleButtonClick() {
	...
	function setNativeValue(element, value) {
		if (!textareaDescriptorCache || !prototypeDescriptorCache) {
			textareaDescriptorCache =
				Object.getOwnPropertyDescriptor(element, 'value') || {};
			const prototype = Object.getPrototypeOf(element);
			prototypeDescriptorCache =
	...
	function CN_StartTTGPT_Prompt() {
		...
		try {
			TTGPT_PROMPT_ALREADY_SHOWN = localStorage.getItem(
				'TTGPT_PROMPT_ALREADY_SHOWN'
			);
		} catch (e) {}
	...
	function CN_InitScript() {
			...
			if (!CN_WANTED_VOICE_NAME) {
				...
				speechSynthesis.getVoices().forEach(function (voice) {
					...
					if (voice.lang + '-' + voice.name == CN_WANTED_VOICE_NAME) {
						...
						console.log(
							'[STARTUP] I will read using voice ' +
								voice.name +
								' (' +
								voice.lang +
								')'
		...
		setTimeout(function () {
			...
			jQuery('.CNToggle, #CNStartButton, #CNResumeButton').on(
				'mouseenter',
				function () {
			...
			jQuery('#TTGPTSettings').mousedown(function (e) {
				...
				function handle_dragging(e) {
					var left =
						my_dragging.offset0.left +
						(e.pageX - my_dragging.pageX0);
					var top =
						my_dragging.offset0.top +
						(e.pageY - my_dragging.pageY0);
		...
		jQuery(document).on('keydown', function (e) {
			...
			if (e.altKey && e.shiftKey && e.which === 83) {
				console.log(
					'[KEY] ALT+SHIFT+S pressed, starting Talk-To-ChatGPT'
				);
				CN_StartTTGPT();
			}
			...
			if (e.altKey && e.shiftKey && e.which === 72) {
				...
				var wantMicOff =
					jQuery('.CNToggle[data-cn=micon]').css('display') == 'none';
				if (wantMicOff) {
					// Turn off bot voice
					console.log(
						'[KEY] ALT+SHIFT+H pressed, turning off speech recognition'
					);
					jQuery('.CNToggle[data-cn=micoff]').click();
				} else {
					// Turn on bot voice
					console.log(
						'[KEY] ALT+SHIFT+H pressed, turning on speech recognition'
					);
			...
			if (e.altKey && e.shiftKey && e.which === 86) {
				...
				var wantSpeakOff =
					jQuery('.CNToggle[data-cn=speakon]').css('display') ==
					'none';
				if (wantSpeakOff) {
					// Turn off bot voice
					console.log(
						'[KEY] ALT+SHIFT+V pressed, turning bot voice off'
					);
					jQuery('.CNToggle[data-cn=speakoff]').click();
				} else {
					// Turn on bot voice
					console.log(
						'[KEY] ALT+SHIFT+V pressed, turning bot voice on'
					);
			...
			if (e.altKey && e.shiftKey && e.which === 76) {
				console.log(
					'[KEY] ALT+SHIFT+L pressed, skipping current message'
				);
	...
	function CN_OnSettingsIconClick() {
		...
		rows +=
			"<table width='100%' cellpadding=6 cellspacing=2 style='margin-top: 15px;'>";
		...
		speechSynthesis.getVoices().forEach(function (voice) {
			...
			var SEL =
				...
				CN_WANTED_VOICE.name == voice.name
					...
					: '';
			voices +=
				"<option value='" + n + "' " + SEL + '>' + label + '</option>';
			n++;
		});
		...
		rows +=
			"<tr><td style='white-space: nowrap'>Speech recognition language:</td><td><select id='TTGPTRecLang' style='width: 250px; padding: 2px; color: black;' >" +
			languages +
			'</select></td></tr>';

		rows +=
			"<tr class='CNBrowserTTS' ><td style='white-space: nowrap'>AI voice and language:</td><td><select id='TTGPTVoice' style='width: 250px; padding: 2px; color: black'>" +
			voices +
			'</select></td></tr>';

		// 2. AI talking speed
		rows +=
			"<tr class='CNBrowserTTS' ><td style='white-space: nowrap'>AI talking speed (speech rate):</td><td><input type=number step='.1' id='TTGPTRate' style='color: black; padding: 2px; width: 100px;' value='" +
			CN_TEXT_TO_SPEECH_RATE +
			"' /></td></tr>";

		// 3. AI voice pitch
		rows +=
			"<tr class='CNBrowserTTS' ><td style='white-space: nowrap'>AI voice pitch:</td><td><input type=number step='.1' id='TTGPTPitch' style='width: 100px; padding: 2px; color: black;' value='" +
			CN_TEXT_TO_SPEECH_PITCH +
			"' /></td></tr>";

		// 4. ElevenLabs
		rows +=
			"<tr><td style='white-space: nowrap'>ElevenLabs text-to-speech:</td><td><input type=checkbox id='TTGPTElevenLabs' " +
			(CN_TTS_ELEVENLABS ? 'checked=checked' : '') +
			" /> <label for='TTGPTElevenLabs'> Use ElevenLabs API for text-to-speech (tick this to reveal additional settings)</label></td></tr>";

		// 5. ElevenLabs API key
		rows +=
			"<tr class='CNElevenLabs' style='display: none;'><td style='white-space: nowrap'>ElevenLabs API Key:</td><td><input type=text style='width: 250px; padding: 2px; color: black;' id='TTGPTElevenLabsKey' value=\"" +
			CN_TTS_ELEVENLABS_APIKEY +
			'" /></td></tr>';

		// 6. ElevenLabs voice
		rows +=
			"<tr class='CNElevenLabs' style='display: none;'><td style='white-space: nowrap'>ElevenLabs voice:</td><td><select id='TTGPTElevenLabsVoice' style='width: 250px; padding: 2px; color: black;' >" +
			"</select> <span style='cursor: pointer; text-decoration: underline;' id='TTGPTElevenLabsRefresh' title='This will refresh the list of voices using your API key'>Refresh list</span></span></td></tr>";

		// 7. ElevenLabs settings
		rows +=
			"<tr class='CNElevenLabs' style='display: none;'><td style='white-space: nowrap'>ElevenLabs settings:</td>" +
			'<td>' +
			"Stability: <input type=number style='width: 100px; padding: 2px; color: black;' step='0.01' min='0' max='1' id='TTGPTElevenLabsStability' value=\"" +
			CN_TTS_ELEVENLABS_STABILITY +
			'" />' +
			"Similarity: <input type=number style='width: 100px; padding: 2px; color: black;' step='0.01' min='0' max='1' id='TTGPTElevenLabsSimilarity' value=\"" +
			CN_TTS_ELEVENLABS_SIMILARITY +
			'" />' +
		...
		rows +=
			'</table><br /><h2>Voice control</h2><b>PLEASE NOTE! </b> These commands only work when the microphone is actively listening / when the speech-to-text functionality is active (indicated by a red bar). ' +
		...
		// 5. 'Stop' word
		rows +=
			"<tr><td style='white-space: nowrap'>'Stop' word:</td><td><input type=text id='TTGPTStopWord' style='width: 100px; padding: 2px; color: black;' value='" +
			CN_SAY_THIS_WORD_TO_STOP +
			"' /></td></tr>";

		// 6. 'Pause' word
		rows +=
			"<tr><td style='white-space: nowrap'>'Pause' word:</td><td><input type=text id='TTGPTPauseWord' style='width: 100px; padding: 2px; color: black;' value='" +
			CN_SAY_THIS_WORD_TO_PAUSE +
			"' /></td></tr>";

		// 7. Keep listening until resume
		rows +=
			"<tr><td style='white-space: nowrap'>Keep listening when paused:</td><td><input type=checkbox id='TTGPTKeepListening' " +
			(CN_KEEP_LISTENING ? 'checked=checked' : '') +
			" /> <label for='TTGPTKeepListening'>When paused, keep the microphone open, and resume conversation when the 'pause' word (defined above) is spoken</label></td></tr>";

		// 8. Autosend
		rows +=
			"<tr><td style='white-space: nowrap'>Automatic send:</td><td><input type=checkbox id='TTGPTAutosend' " +
		...
		// 9. Manual send word
		rows +=
			"<tr><td style='white-space: nowrap'>Manual send word(s):</td><td><input type=text id='TTGPTSendWord' style='width: 250px; padding: 2px; color: black;' value='" +
			CN_SAY_THIS_TO_SEND +
			"' /><span style='font-size: 10px;'>If 'automatic send' is disabled, you can trigger the sending of the message by saying this word (or sequence of words)</span></td></tr>";
		...
		rows +=
			"<tr><td style='white-space: nowrap'>Punctuation in sentences:</td><td><input type=checkbox id='TTGPTIgnoreCommas' " +
			(CN_IGNORE_COMMAS ? 'checked=checked' : '') +
			" /> <label for='TTGPTIgnoreCommas'>Don't use commas/semicolons/etc. to break down replies into sentences</label></td></tr>";

		// 11. Ignore code blocks
		rows +=
			"<tr><td style='white-space: nowrap'>Ignore code blocks:</td><td><input type=checkbox id='TTGPTIgnoreCode' " +
			(CN_IGNORE_CODE_BLOCKS ? 'checked=checked' : '') +
			" /> <label for='TTGPTIgnoreCode'>Don't read blocks of code out loud (ignore them altogether)</label></td></tr>";

		// 12. AI Speak Emojis or not
		rows +=
			"<tr><td style='white-space: nowrap'>AI Speak Emojis:</td><td><input type=checkbox id='TTGPTSpeakEmojis' " +
			(CN_SPEAK_EMOJIS ? 'checked=checked' : '') +
			" /> <label for='TTGPTSpeakEmojis'> Allow the bot to describe emojis (e.g. 'smiling face with heart eyes')</label> <span style='font-size: 10px;'>This setting doesn't apply if you have the ElevenLabs text-to-speech enabled (ElevenLabs ignores emojis)</span></td></tr>";

		// Keyboard shortcuts
		rows +=
			"<tr><td style='white-space: nowrap'>Keyboard shortcuts:</td><td><ul>" +
		...
		// Add donations frame
		var donations =
			"<br/><h2>Support the project</h2><p style='font-size: 15px; margin-top: 15px;'>Are you enjoying Talk-To-ChatGPT and want me to continue improving it? \n" +
		...
		jQuery('body').append(
			"<div style='background: rgba(0,0,0,0.8); position: absolute; overflow-y: auto; top: 0; right: 0; left: 0; bottom: 0; z-index: 999999; padding: 20px; color: white; font-size: 13px;' id='TTGPTSettingsArea'>" +
				"<div style='width: 600px; margin-left: auto; margin-right: auto; overflow-y: auto;'><h1>⚙️ Talk-to-ChatGPT settings</h1>" +
				desc +
				rows +
				donations +
				'</div></div>'
	...
	function CN_SaveSettings() {
		...
		try {
			...
			CN_WANTED_VOICE_NAME = CN_WANTED_VOICE
				? CN_WANTED_VOICE.lang + '-' + CN_WANTED_VOICE.name
				: '';
			...
			CN_SAY_THIS_WORD_TO_STOP = CN_RemovePunctuation(
				jQuery('#TTGPTStopWord').val()
			);
			CN_SAY_THIS_WORD_TO_PAUSE = CN_RemovePunctuation(
				jQuery('#TTGPTPauseWord').val()
			);
			CN_KEEP_LISTENING = jQuery('#TTGPTKeepListening').prop('checked');
			CN_AUTO_SEND_AFTER_SPEAKING =
				jQuery('#TTGPTAutosend').prop('checked');
			CN_SAY_THIS_TO_SEND = CN_RemovePunctuation(
				jQuery('#TTGPTSendWord').val()
			);
			...
			CN_TTS_ELEVENLABS_APIKEY = CN_RemovePunctuation(
				jQuery('#TTGPTElevenLabsKey').val() + ''
			);
			CN_TTS_ELEVENLABS_VOICE =
				jQuery('#TTGPTElevenLabsVoice').val() + '';
			CN_TTS_ELEVENLABS_STABILITY = jQuery(
				'#TTGPTElevenLabsStability'
			).val();
			CN_TTS_ELEVENLABS_SIMILARITY = jQuery(
				'#TTGPTElevenLabsSimilarity'
			).val();

			// If ElevenLabs is active, and that there is no voice, error out
			if (CN_TTS_ELEVENLABS && !CN_TTS_ELEVENLABS_VOICE) {
				alert(
					'To enable ElevenLabs support, you must select a voice in the dropdown list. Click the Refresh List button. If no voice appears in the list, check your API key. If you are 100% sure your API key is valid, please report the issue on the Github project page, on the Issues tab.'
				);
				return;
			}
	...
	function CN_RestoreSettings() {
		...
		try {
			...
			if (typeof settings == 'object' && settings != null) {
				console.log(
					'[COOKIE] Reloading settings from cookie: ' + settings
				);
				...
				if (settings.hasOwnProperty(16))
					CN_SPEAK_EMOJIS = settings[16] == 1;
			}
		} catch (ex) {
			console.error(ex);
		}
	...
	function CN_RefreshElevenLabsVoiceList(useKeyFromTextField) {
		...
		xhr.setRequestHeader('Content-Type', 'application/json');
		var apikey = useKeyFromTextField
			? jQuery('#TTGPTElevenLabsKey').val()
			: CN_TTS_ELEVENLABS_APIKEY;
		...
		xhr.onreadystatechange = function () {
			...
			if (xhr.readyState === XMLHttpRequest.DONE) {
				...
				try {
					...
					jQuery('#TTGPTElevenLabsRefresh').html('Refresh list');
					alert(
						'Error retrieving ElevenLabs voice list: ' +
							e.toString() +
							'. Please ensure you have a valid API key and try clicking Refresh List again.'
					);
					return;
				}
				...
				if (typeof result.voices == 'undefined') {
					if (
						typeof result.detail != 'undefined' &&
						typeof result.detail.message != 'undefined'
					) {
						// {"detail":{"status":"invalid_api_key","message":"Invalid API key: 'apikey'"}}
						alert(
							'ElevenLabs returned the following while refreshing the voice list: ' +
								result.detail.message
						);
						return;
					}
					// Other
					alert(
						'Unexpected response from ElevenLabs API: ' +
				...
				for (var modelId in CN_TTS_ELEVENLABS_MODELS) {
					...
					for (var i = 0; i < result.voices.length; i++) {
						...
						// Add to proper list
						var isMultiling =
							typeof result.voices[i]
								.high_quality_base_model_ids == 'object'
								? result.voices[i].high_quality_base_model_ids
										.length
								: 0;
				...
				if (CN_TTS_ELEVENLABS_VOICE && !found)
					alert(
						"The voice previously selected in the settings doesn't seem to be available in your ElevenLabs account anymore. Please select a new voice in the settings to restore ElevenLabs support. Voice ID: " +
							CN_TTS_ELEVENLABS_VOICE
					);
			}
		};
	...
	(function () {
		setTimeout(function () {
			typeof jQuery == 'undefined'
				? alert(
						'[Talk-to-ChatGPT] Sorry, but jQuery was not able to load. The script cannot run. Try using Google Chrome or Edge on Windows 11'
				  )
				: CN_CheckCorrectPage();
		}, 500);
	})();
